(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('preact'), require('preact/hooks'), require('@preact/signals')) :
  typeof define === 'function' && define.amd ? define(['exports', 'preact', 'preact/hooks', '@preact/signals'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global["@schedule-x/calendar"] = {}, global.preact, global.hooks, global.signals));
})(this, (function (exports, preact, hooks, signals) { 'use strict';
  ...
  function AppWrapper({ $app }) {
      ...
      return (u(preact.Fragment, { children: u("div", { className: classes.join(' '), children: u(AppContext$1.Provider, { value: $app, children: [u(AppInput, {}), $app.datePickerState.isOpen.value && u(AppPopup, {})] }) }) }));
  }
  ...
  function ForwardBackwardNavigation() {
      ...
      return (u(preact.Fragment, { children: u("div", { className: "sx__forward-backward-navigation", "aria-label": localizedRange, "aria-live": "polite", children: [u(Chevron, { onClick: () => navigate('backwards'), direction: 'previous', buttonText: $app.translate('Previous period') }), u(Chevron, { onClick: () => navigate('forwards'), direction: 'next', buttonText: $app.translate('Next period') })] }) }));
  }
  ...
  function CalendarWrapper({ $app }) {
      ...
      hooks.useEffect(renderSelectedView, [$app.calendarState.view.value]);
      ...
      return (u(preact.Fragment, { children: u("div", { className: wrapperClasses.join(' '), id: calendarId, children: u("div", { className: 'sx__calendar', children: u(AppContext.Provider, { value: $app, children: [u(CalendarHeader, {}), u("div", { className: ['sx__view-container', transitionClass].join(' '), id: viewContainerId }), $app.config.plugins.eventModal &&
                              $app.config.plugins.eventModal.calendarEvent.value && (u($app.config.plugins.eventModal.ComponentFn, { "$app": $app }))] }) }) }) }));
  }
  ...
  const createCalendarState = (calendarConfig, timeUnitsImpl) => {
      var _a;
      const view = signals.signal(((_a = calendarConfig.views.find((view) => view.name === calendarConfig.defaultView)) === null || _a === void 0 ? void 0 : _a.name) || calendarConfig.views[0].name);
      ...
      const isDark = signals.signal(calendarConfig.isDark || false);
      signals.effect(() => {
  ...
  const createDatePickerConfig = (config, dateSelectionCallback) => {
      ...
      return new ConfigBuilder()
          .withLocale(config.locale)
          .withFirstDayOfWeek(config.firstDayOfWeek)
          .withMin((_a = config.datePicker) === null || _a === void 0 ? void 0 : _a.min)
          .withMax((_b = config.datePicker) === null || _b === void 0 ? void 0 : _b.max)
          .withStyle((_c = config.datePicker) === null || _c === void 0 ? void 0 : _c.style)
  ...
  const getEventTop = (start, dayBoundaries, pointsPerDay) => {
  ...
  function UserIcon({ strokeColor }) {
      return (u(preact.Fragment, { children: u("svg", { className: "sx__event-icon", viewBox: "0 0 24 24", fill: "none", xmlns: "http://www.w3.org/2000/svg", children: [u("g", { id: "SVGRepo_bgCarrier", "stroke-width": "0" }), u("g", { id: "SVGRepo_tracerCarrier", "stroke-linecap": "round", "stroke-linejoin": "round" }), u("g", { id: "SVGRepo_iconCarrier", children: [u("path", { d: "M15 7C15 8.65685 13.6569 10 12 10C10.3431 10 9 8.65685 9 7C9 5.34315 10.3431 4 12 4C13.6569 4 15 5.34315 15 7Z", stroke: strokeColor, "stroke-width": "2" }), u("path", { d: "M5 19.5C5 15.9101 7.91015 13 11.5 13H12.5C16.0899 13 19 15.9101 19 19.5V20C19 20.5523 18.5523 21 18 21H6C5.44772 21 5 20.5523 5 20V19.5Z", stroke: strokeColor, "stroke-width": "2" })] })] }) }));
  }
  ...
  function TimeIcon({ strokeColor }) {
      return (u(preact.Fragment, { children: u("svg", { className: "sx__event-icon", viewBox: "0 0 24 24", fill: "none", xmlns: "http://www.w3.org/2000/svg", children: [u("g", { id: "SVGRepo_bgCarrier", "stroke-width": "0" }), u("g", { id: "SVGRepo_tracerCarrier", "stroke-linecap": "round", "stroke-linejoin": "round" }), u("g", { id: "SVGRepo_iconCarrier", children: [u("path", { d: "M12 8V12L15 15", stroke: strokeColor, "stroke-width": "2", "stroke-linecap": "round" }), u("circle", { cx: "12", cy: "12", r: "9", stroke: strokeColor, "stroke-width": "2" })] })] }) }));
  }
  ...
  const getCCID = (customComponent, calendarEvent, isCopy) => {
  ...
  function TimeGridEvent({ calendarEvent, dayBoundariesDateTime, isCopy, }) {
      ...
      return (u(preact.Fragment, { children: [u("div", { id: isCopy ? getTimeGridEventCopyElementId(calendarEvent.id) : undefined, onClick: handleOnClick, onMouseDown: (e) => createDragStartTimeout(handleStartDrag, e), onMouseUp: (e) => setClickedEventIfNotDragging(calendarEvent, e), onTouchStart: (e) => createDragStartTimeout(handleStartDrag, e), onTouchEnd: (e) => setClickedEventIfNotDragging(calendarEvent, e), className: 'sx__time-grid-event sx__event' + (isCopy ? ' is-event-copy' : ''), tabIndex: 0, style: {
                  ...
                  }, children: u("div", { "data-ccid": customComponentId, className: "sx__time-grid-event-inner", children: [!customComponent && (u(preact.Fragment, { children: [calendarEvent.title && (u("div", { className: "sx__time-grid-event-title", children: calendarEvent.title })), u("div", { className: "sx__time-grid-event-time", children: [u(TimeIcon, { strokeColor: eventCSSVariables.iconStroke }), getEventTime(calendarEvent.start, calendarEvent.end)] }), calendarEvent.people && (u("div", { className: "sx__time-grid-event-people", children: [u(UserIcon, { strokeColor: eventCSSVariables.iconStroke }), concatenatePeople(calendarEvent.people)] }))] })), $app.config.plugins.resize && (u("div", { className: 'sx__time-grid-event-resize-handle', onMouseDown: startResize }))] }) }), eventCopy && u(TimeGridEvent, { calendarEvent: eventCopy, isCopy: true })] }));
  }
  ...
  function TimeGridDay({ calendarEvents, date }) {
      ...
      return (u("div", { className: "sx__time-grid-day", onClick: handleOnClick, "aria-label": getLocalizedDate(date, $app.config.locale), children: eventsWithConcurrency.map((event) => (u(TimeGridEvent, { calendarEvent: event, dayBoundariesDateTime: dayBoundariesDateTime }, event.id))) }));
  }
  ...
  function TimeAxis() {
      ...
      return (u(preact.Fragment, { children: u("div", { className: "sx__week-grid__time-axis", children: hours.map((hour) => (u("div", { className: "sx__week-grid__hour", children: u("span", { className: "sx__week-grid__hour-text", children: new Date(0, 0, 0, hour).toLocaleTimeString($app.config.locale, {
                          hour: 'numeric',
                      }) }) }))) }) }));
  }
  ...
  function DateAxis({ week }) {
      ...
      return (u(preact.Fragment, { children: u("div", { className: "sx__week-grid__date-axis", children: week.map((date) => (u("div", { className: getClassNames(date), children: [u("div", { className: "sx__week-grid__day-name", children: getDayNameShort(date, $app.config.locale) }), u("div", { className: "sx__week-grid__date-number", children: date.getDate() })] }))) }) }));
  }
  ...
  function DateGridEvent({ calendarEvent, gridRow, isCopy, }) {
      ...
      const borderLeftNonCustom = hasOverflowLeft
          ? 'none'
          : eventCSSVariables.borderLeft;
      return (u(preact.Fragment, { children: [u("div", { id: isCopy ? getTimeGridEventCopyElementId(calendarEvent.id) : undefined, tabIndex: 0, "aria-label": calendarEvent.title +
                      ' ' +
                      getTimeStamp(calendarEvent, $app.config.locale, $app.translate('to')), "data-ccid": customComponentId, onMouseDown: (e) => createDragStartTimeout(handleStartDrag, e), onMouseUp: (e) => setClickedEventIfNotDragging(calendarEvent, e), onTouchStart: (e) => createDragStartTimeout(handleStartDrag, e), onTouchEnd: (e) => setClickedEventIfNotDragging(calendarEvent, e), onClick: () => invokeOnEventClickCallback($app, calendarEvent), className: eventClasses.join(' '), style: {
                  ...
                  }, children: [!customComponent && (u(preact.Fragment, { children: [hasOverflowLeft && (u("div", { className: 'sx__date-grid-event--left-overflow', style: overflowStyles })), u("span", { className: "sx__date-grid-event-text", children: calendarEvent.title }), hasOverflowRight && (u("div", { className: 'sx__date-grid-event--right-overflow', style: overflowStyles }))] })), $app.config.plugins.resize && !hasOverflowRight && (u("div", { className: "sx__date-grid-event-resize-handle", onMouseDown: startResize }))] }), eventCopy && (u(DateGridEvent, { calendarEvent: eventCopy, gridRow: gridRow, isCopy: true }))] }));
  }
  ...
  function MonthAgendaEvent({ calendarEvent }) {
      ...
      return (u("div", { className: "sx__event sx__month-agenda-event", "data-ccid": customComponentId, style: {
              ...
              borderLeft: customComponent ? undefined : eventCSSVariables.borderLeft,
              padding: customComponent ? '0px' : undefined,
          }, onClick: (e) => onClick(e), children: !customComponent && (u(preact.Fragment, { children: [u("div", { className: "sx__month-agenda-event__title", children: calendarEvent.title }), u("div", { className: "sx__month-agenda-event__time sx__month-agenda-event__has-icon", children: [u(TimeIcon, { strokeColor: `var(--sx-color-on-${calendarEvent._color}-container)` }), getTimeStamp(calendarEvent, $app.config.locale)] })] })) }));
  }
  ...
  const MonthAgendaWrapper = ({ $app, id }) => {
      ...
      const [activeDate, setActiveDate] = hooks.useState($app.datePickerState.selectedDate.value);
      return (u(AppContext.Provider, { value: $app, children: u("div", { id: id, className: "sx__month-agenda-wrapper", children: [u(MonthAgendaDayNames, { week: agendaMonth.weeks[0] }), u("div", { className: "sx__month-agenda-weeks", children: agendaMonth.weeks.map((week) => (u(MonthAgendaWeek, { week: week, setActiveDate: setActiveDate, activeDate: activeDate }))) }), u(MonthAgendaEvents, { events: ((_a = agendaMonth.weeks.flat().find((day) => day.date === activeDate)) === null || _a === void 0 ? void 0 : _a.events) || [] }, activeDate)] }) }));
  };
